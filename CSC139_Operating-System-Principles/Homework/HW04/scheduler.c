...
void rr(){
    ...
    printf("\nProcess list for Round Robin in order entered:\n");
    for (i = 0; i < numOfProcs; i++) {
        printf("%d %d %d\n", readyQ[i].id, readyQ[i].timeNeeded, readyQ[i].priority);
    ...
    //overhead = 0;    
        ...
        for(i = 0; i < numOfProcs; i++) {
            ...
            //printf("RR TA for finished p%d = %d, needed: %d ms, and %d time slices.\n", readyQ[i - 1].id, readyQ[i - 1].turnAround, rawInput[i - 1].timeNeeded, readyQ[i - 1].slices);
    ...
    printf("<><> End Preeptive Round Robin Schedule <><>\n\n");
}
