		...
		stats_t *race_stats )             /* in & out */
{
    ...
    for (i = 0; i < NRACERS; i++)
    {
        ...
        for (j = 0; j < TRIES; j++)
        {
            ...
            {
                driver_list[i].tries[j] = driver_list[i].tries[j];
            }
		...
		stats_t *race_stats )
{
    ...
    {
        double middle = driver_list[midPoint].best_time + driver_list[midPoint - 1].best_time;
        race_stats -> median = middle / 2.0;
    }
}
